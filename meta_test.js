...
class BattleSimulator {
    ...
    applyDamage(attacker, defender, dmg, spd, useIdx, isBonda) {
        ...
        for (let d = 0; d < dmg; d++) {
            if (defender.musou === 1 || defender.bouheki > 0) { 
...
// 検証
const trials = 10000;
const combos = ["連防", "連連", "防防", "連強", "連速"];
const bosses = [
    { name: "多段型", skills: "雷雷果果硬硬果雷待硬" },
    { name: "妨害型", skills: "影封搦待待硬封影刺硬" },
    { name: "汚染型", skills: "疫疫硬硬果果疫硬待硬" },
    { name: "絶望型", skills: "疫影封待雷果硬硬硬硬" }
];

bosses.forEach(boss => {
    console.log(`\nTesting Boss: ${boss.name} (${boss.skills})`);
    combos.forEach(combo => {
        let wins = 0;
        for (let i = 0; i < trials; i++) {
            let pSkills = combo;
            const pool = "一刺果剣紫呪雷隠怒弱覚防封影交搦待玉崩疫強硬速反盾錬逆無裏先連燐空";
            while (pSkills.length < 5) pSkills += pool[Math.floor(Math.random() * pool.length)];
            if (new BattleSimulator(new Player(pSkills, "P"), new Player(boss.skills, "B")).start() === 1) wins++;
        }
        console.log(`  vs ${combo}: Win Rate ${(wins / trials * 100).toFixed(2)}%`);
    });
});
