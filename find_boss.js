...
class BattleSimulator {
    ...
    unitTurn(attacker, defender, prior) {
        ...
        if (useIdx === -1) { this.applyDamage(attacker, defender, 1, 0, -1, true); }
            ...
            // リミテッドスキルの処理: 使用した時点でリミテッドなら scar = 2 (一時的使用済み)
            ...
            if (useIdx < attacker.getSkillsLength() - 1 && attacker.skill[useIdx+1] === "盾") {
                attacker.tate = 2;
                attacker.limited[useIdx+1] = 2; // ＋盾もリミテッド
            }
        }
            ...
            if (tIdx !== -1) {
                ...
                if (defender.type[tIdx] === COUNTER || (defender.type[tIdx] === ATTACK && tIdx < defender.getSkillsLength()-1 && defender.skill[tIdx+1] === "反")) {
                    if (!(useIdx !== -1 && useIdx < attacker.getSkillsLength()-1 && attacker.skill[useIdx+1] === "錬")) {
                        ...
                        if (cSpd >= spd || isBonda) {
                            if (defender.limited[tIdx] === 1) defender.limited[tIdx] = 2; // 迎撃発動でリミテッド
    ...
    processEndPhase(pc) {
        ...
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.limited[i] === 2) {
                pc.limited[i] = 1; pc.scar[i] = 1; // ターン終了時に破壊フラグ
            }
        }
        if (pc.suijaku === 1) {
            for (let i = 0; i < pc.getSkillsLength(); i++) {
        ...
        this.breakup(pc); // ターン終了時のリミテッド破棄を適用
    }
}
...
const pool = "一果待崩刺搦速防硬盾剣交強";
const patterns = getAllSetups(pool);
const bossOptions = [
    "崩待防果速", "崩待果強一", "崩待刺一果"
];
...
bossOptions.forEach(boss => {
    ...
    patterns.forEach(setup => {
        if (new BattleSimulator(new Player(setup, "P"), new Player(boss, "B")).start() === 1) {
            wins++; if (winningSetups.length < 3) winningSetups.push(setup);
        }
    });
    console.log(`Boss: ${boss}, Win Rate: ${(wins / patterns.length * 100).toFixed(2)}%, Samples: ${winningSetups.join(', ')}`);
});
