...
var StatusFlag;
(function (StatusFlag) {
    StatusFlag[StatusFlag["NONE"] = 0] = "NONE";
    StatusFlag[StatusFlag["ACTIVE"] = 1] = "ACTIVE";
    StatusFlag[StatusFlag["RESERVED"] = 2] = "RESERVED";
    StatusFlag[StatusFlag["FROZEN"] = 3] = "FROZEN";
...
(function (BattleResult) {
    BattleResult[BattleResult["CONTINUE"] = 0] = "CONTINUE";
    BattleResult[BattleResult["PC1_WIN"] = 1] = "PC1_WIN";
    BattleResult[BattleResult["PC2_WIN"] = 2] = "PC2_WIN";
    BattleResult[BattleResult["DRAW"] = 3] = "DRAW";
})(BattleResult || (BattleResult = {}));
class Battle {
    constructor(pc1Data, pc2Data) {
        this.text = "";
        this.turn = 1;
        this.roundFirst = null;
        this.dataPc1 = pc1Data;
        this.dataPc2 = pc2Data;
        this.pc1 = new Player_1.Player(this.dataPc1);
        this.pc2 = new Player_1.Player(this.dataPc2);
    }
    /**
     * ログを出力する
     */
    ...
    executeTurn() {
        ...
        if (masteryResult !== BattleResult.CONTINUE)
    ...
    executeGouyoku() {
        const applyGouyoku = (pc, opponent, isFirst) => {
            if (pc.gouyoku > 0) {
                this.log();
    ...
    showPlayerStatus(pc) {
        ...
        let statusText = "";
        const appendStatus = (label, count) => {
            if (count > 0)
                statusText += `${label}x${count}`;
        };
    ...
    applyRinko(pc) {
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.skill[i] === "燐") {
                ...
                if (pc.stan === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}のスタンが解除された！`);
                    pc.stan = StatusFlag.NONE;
                }
                if (pc.roubai === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}の狼狽が解除された！`);
                    pc.roubai = StatusFlag.NONE;
                }
                if (pc.suijaku === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}の忘却が解除された！`);
                    pc.suijaku = StatusFlag.NONE;
                }
                if (pc.tyudoku === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}の毒が解除された！`);
                    pc.tyudoku = StatusFlag.NONE;
                }
                if (pc.yakedo === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}の火傷が解除された！`);
                    pc.yakedo = StatusFlag.NONE;
                }
                if (pc.shimon > 0) {
                    this.log(`＞${pc.playerName}の死紋が解除された！`);
                    pc.shimon = StatusFlag.NONE;
                }
                if (pc.shitto === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}の嫉妬が解除された！`);
                    pc.shitto = StatusFlag.NONE;
                }
                if (pc.shikiyoku === StatusFlag.ACTIVE) {
                    this.log(`＞${pc.playerName}の色欲が解除された！`);
                    pc.shikiyoku = StatusFlag.NONE;
                }
                this.log();
                break;
            }
    ...
    updateTimers(pc) {
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.kage[i] === StatusFlag.ACTIVE) {
                pc.kage[i] = StatusFlag.NONE;
                pc.scar[i] = 1;
            }
            if (pc.limited[i] === StatusFlag.RESERVED) {
                pc.limited[i] = StatusFlag.ACTIVE;
                pc.scar[i] = 1;
            }
        }
    }
    applyPoisonDamage(pc) {
        if (pc.tyudoku !== StatusFlag.ACTIVE)
            return 0;
        this.log(`＞${pc.playerName}は毒に蝕まれている！`);
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.type[i] !== Player_1.Player.NONE && pc.scar[i] === 0) {
    ...
    findSkillByLevel(pc, level) {
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.getSkillLevel(i) === level)
                return i;
        }
        return -1;
    }
    ...
    calculateCurrentSpeed(pc, opponent) {
        if (pc.stan === StatusFlag.ACTIVE && opponent.stan === StatusFlag.NONE)
            return -999;
        if (pc.sensei === StatusFlag.ACTIVE && opponent.sensei === StatusFlag.NONE)
            return 999;
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            // 発動可能な攻撃または補助スキルを探す
            if (this.isActionableSkill(pc, i)) {
                ...
                if (i < pc.getSkillsLength() - 1) {
                    if (pc.skill[i + 1] === "速")
                        speedBuf += 1;
                    if (pc.type[i] === Player_1.Player.ATTACK && pc.skill[i + 1] === "弓")
                        speedBuf += 3;
                }
                // 状態異常・バフ
                if (pc.kakugo === StatusFlag.ACTIVE)
                    speedBuf += 2;
                if (pc.gouman > 0)
                    speedBuf += 1;
                // 相手の座礁チェック
                for (let k = 0; k < opponent.getSkillsLength(); k++) {
                    if (opponent.skill[k] === "礁")
                        speedBuf -= 1;
                }
    ...
    isActionableSkill(pc, idx) {
        const type = pc.type[idx];
        const skill = pc.skill[idx];
        if (type === Player_1.Player.NONE)
            return false;
        // リミテッド使用済みチェック
        if (pc.limited[idx] === StatusFlag.RESERVED)
            return false;
        // 攻撃スキルの場合
        if (type === Player_1.Player.ATTACK) {
            // 反撃付帯がついていないこと
            const hasCounter = (idx < pc.getSkillsLength() - 1 && pc.skill[idx + 1] === "反");
            // 隠刃の隔ターン判定
            const isHiddenNotActive = (skill === "隠" && this.turn % 2 !== 0);
            if (!hasCounter && !isHiddenNotActive)
                return true;
        }
        // 補助スキルの場合
        if (type === Player_1.Player.BUFF)
            return true;
        return false;
    }
    /**
     * 攻撃フェイズ
     */
    phaseAction(attacker, defender, isFirst) {
        let skillIdx = -1;
        ...
        // 攻撃後のエフェクト適用と破壊
        if (this.applyEffects(attacker, defender) + this.applyEffects(defender, attacker) >= 1)
            this.log();
        if (this.applyBreakup(attacker) + this.applyBreakup(defender) >= 1)
            this.log();
    }
    processSkillEffects(attacker, defender, idx, isFirst) {
        // リミテッド処理
        if (attacker.limited[idx] === StatusFlag.ACTIVE)
            attacker.limited[idx] = StatusFlag.RESERVED;
        // ＋弓によるリミテッド化
        if (attacker.type[idx] === Player_1.Player.ATTACK && idx < attacker.getSkillsLength() - 1 && attacker.skill[idx + 1] === "弓") {
            attacker.limited[idx] = StatusFlag.RESERVED;
        }
        // ダメージと速度の計算
        ...
        if (idx < attacker.getSkillsLength() - 1 && !noEnchant) {
            ...
            if (nextSkill === "強") {
                this.log(`＞【＋強】${idx + 2}の効果でダメージが1点上昇！`);
                damageBuf++;
            }
            if (nextSkill === "速")
                this.log(`＞【＋速】${idx + 2}の効果で速度に+1！`);
            if (attacker.type[idx] === Player_1.Player.ATTACK && nextSkill === "錬") {
                this.log(`＞【＋錬】${idx + 2}の効果で迎撃スキルの発動を1回のみ無効に！`);
            ...
            if (nextSkill === "光" && attacker.skill[idx] === "一") {
                this.log(`＞【＋光】${idx + 2}の効果でダメージが2点上昇！`);
                damageBuf += 2;
            }
        }
        if (attacker.kakugo === StatusFlag.ACTIVE) {
            this.log("＞覚悟の効果でダメージが1点上昇！速度に+2！");
            damageBuf++;
        }
        if (attacker.kyousou > 0) {
            this.log(`＞協奏の効果でダメージが${attacker.kyousou}点上昇！速度に+${attacker.kyousou}！`);
            damageBuf += attacker.kyousou;
        }
        if (attacker.funnu > 0) {
            this.log("＞憤怒の効果でダメージが1点上昇！");
            damageBuf++;
        }
        if (attacker.gekirin > 0) {
            this.log(`＞逆鱗の効果でダメージが${attacker.gekirin}点上昇！`);
            damageBuf += attacker.gekirin;
            attacker.gekirin = 0;
        }
        if (attacker.roubai === StatusFlag.ACTIVE)
            this.log("＞狼狽の効果で速度が0になっている！");
        if (attacker.yakedo === StatusFlag.ACTIVE) {
            this.log("＞火傷の効果でダメージが1下がった！");
            damageBuf--;
        }
        if (attacker.shikiyoku > 0) {
            this.log("＞色欲の効果でダメージが1下がった！");
            damageBuf--;
        }
        if (defender.shimon > 0) {
            this.log(`＞死紋の効果で${defender.playerName}へのダメージが${defender.shimon}点上昇！`);
            damageBuf += defender.shimon;
        }
        // 最終ダメージ・速度
        let finalDamage = Math.max(0, damage + damageBuf);
        if ((attacker.yakedo === StatusFlag.ACTIVE || attacker.shikiyoku > 0) && finalDamage < 1 && damage > 0)
            finalDamage = 1; // 最低1 (火傷、色欲)
        let finalSpeed = speed;
        if (idx < attacker.getSkillsLength() - 1 && attacker.skill[idx + 1] === "速" && !noEnchant)
            finalSpeed += 1;
        // ＋弓の効果
        if (attacker.type[idx] === Player_1.Player.ATTACK && idx < attacker.getSkillsLength() - 1 && attacker.skill[idx + 1] === "弓") {
        ...
        if (attacker.kakugo === StatusFlag.ACTIVE)
            finalSpeed += 2;
        if (attacker.kyousou > 0)
            finalSpeed += attacker.kyousou;
        if (attacker.gouman > 0)
            finalSpeed += 1;
        for (let k = 0; k < defender.getSkillsLength(); k++) {
            if (defender.skill[k] === "礁")
                finalSpeed = Math.max(finalSpeed - 1, 0);
        }
        if (attacker.roubai === StatusFlag.ACTIVE)
            finalSpeed = 0;
        // 攻撃の実行
        if (attacker.type[idx] === Player_1.Player.BUFF) {
            ...
            if (attacker.skill[idx] === "滅")
                this.clearGoodStatuses(defender, "撃滅");
        ...
        // 強欲の更新
        if (attacker.skill[idx] === "欲")
            attacker.gouyoku = StatusFlag.RESERVED;
    }
    handleBuffSkill(attacker, defender, idx) {
        ...
        switch (skill) {
            ...
            case "癒":
                if (idx >= 0)
                    attacker.chiyuReserved.push(Math.max(attacker.getSkillLevel(idx) - 1, 0));
                break;
            case "凍":
                defender.roubai = StatusFlag.RESERVED;
                if (idx >= 0)
                    attacker.limited[idx] = StatusFlag.FROZEN; // 終了フェイズ処理のため
                break;
            case "魚":
                this.log(`＞${defender.playerName}に1点のダメージ！（迎撃不可）`);
                attacker.gyogun = 1;
                break;
            case "盗":
                this.log("＞盗賊の効果！");
                ...
                break;
            case "覚":
                attacker.kakugo = StatusFlag.RESERVED;
                break;
            case "防":
                attacker.bouheki_ = StatusFlag.RESERVED;
                break;
            case "毒":
                defender.tyudoku = StatusFlag.RESERVED;
                break;
            case "影":
    ...
    calculateSkillDamage(attacker, defender, idx, isFirst, incomingSpeed) {
        switch (attacker.skill[idx]) {
            case "剣": {
                let damage = 0;
                for (let j = 0; j < attacker.getSkillsLength(); j++) {
                    if (attacker.type[j] === Player_1.Player.ATTACK && !(j < attacker.getSkillsLength() - 1 && attacker.skill[j + 1] === "反")) {
                        damage++;
                    }
                }
                return damage;
            ...
            case "瘴": {
                let damage = 0;
                for (let j = 0; j < attacker.getSkillsLength(); j++)
                    if (attacker.skill[j] === "疫")
                        damage++;
                for (let j = 0; j < defender.getSkillsLength(); j++)
                    if (defender.skill[j] === "疫")
                        damage++;
                return damage;
            }
            case "拳":
            ...
            case "死": {
                ...
                if (defender.stan > 0)
                    damage++;
                if (defender.roubai > 0)
                    damage++;
                if (defender.suijaku > 0)
                    damage++;
                if (defender.tyudoku > 0)
                    damage++;
                if (defender.yakedo > 0)
                    damage++;
                if (defender.shimon > 0)
                    damage += defender.shimon;
                if (defender.shitto > 0)
                    damage++;
                if (defender.shikiyoku > 0)
                    damage++;
                return damage;
            }
            case "狼":
            ...
            case "憤": {
                ...
                for (let k = 0; k < attacker.getSkillsLength(); k++) {
                    if (attacker.type[k] === Player_1.Player.NONE && attacker.skill[k] !== "空" && attacker.skill[k] !== "＿")
                        damage++;
                }
                return damage;
            }
    ...
    clearGoodStatuses(pc, sourceName) {
        const hadGoodStatus = pc.kakugo === StatusFlag.ACTIVE ||
            pc.bouheki > 0 ||
            pc.musou === StatusFlag.ACTIVE ||
            pc.sensei === StatusFlag.ACTIVE ||
            pc.kaifuku > 0 ||
        ...
        pc.reika = 0;
        if (hadGoodStatus)
            this.log(`＞${sourceName}の効果で${pc.playerName}の良い状態が全て解除された！`);
    }
    reserveSelfSkillEffects(attacker, idx) {
    ...
    reserveKageuchi(defender) {
        for (let i = 0; i < defender.getSkillsLength(); i++) {
            if (defender.type[i] !== Player_1.Player.NONE) {
    ...
    getStatusName(key) {
        const names = { tyudoku: "毒", yakedo: "火傷", roubai: "狼狽", stan: "スタン", suijaku: "忘却" };
        return names[key] || key;
    }
    executeAttack(attacker, defender, useIdx, damage, speed) {
        if (damage > 0)
            this.log(`＞${defender.playerName}に${damage}点のダメージ！（速度：${speed}）`);
        this.log();
        let isSuspended = false;
        const counterSuppressions = this.getCounterSuppressionSources(attacker, useIdx);
        for (let d = 0; d < damage; d++) {
            if (defender.musou === StatusFlag.ACTIVE) {
                this.log(`＞${defender.playerName}は無想の効果でダメージを受けない！`);
                break;
            }
    ...
    getCounterSuppressionSources(attacker, useIdx) {
        if (useIdx === -1)
            return [];
        const sources = [];
        if (attacker.skill[useIdx] === "狼")
            sources.push(useIdx);
        if (attacker.type[useIdx] === Player_1.Player.ATTACK && useIdx < attacker.getSkillsLength() - 1 && attacker.skill[useIdx + 1] === "錬")
            sources.push(useIdx + 1);
        return sources;
    }
    selectTarget(attacker, defender, useIdx) {
        // 刺突・艦砲の特殊ターゲット
        ...
        for (let i = 0; i < defender.getSkillsLength(); i++) {
            if (defender.type[i] !== Player_1.Player.NONE && defender.scar[i] !== 2)
                return i;
        }
        return -1;
    }
    selectPiercingTarget(defender, level) {
        let bestDiff = 999;
        let bestIdx = -1;
        for (let i = 0; i < defender.getSkillsLength(); i++) {
            if (defender.type[i] !== Player_1.Player.NONE && defender.scar[i] !== 2) {
    ...
    handleCounter(attacker, defender, useIdx, targetIdx, attackerSpeed, counterSuppressions) {
        const isCounterSkill = (defender.type[targetIdx] === Player_1.Player.COUNTER ||
            (defender.type[targetIdx] === Player_1.Player.ATTACK && targetIdx < defender.getSkillsLength() - 1 && defender.skill[targetIdx + 1] === "反"));
        if (!isCounterSkill)
            return false;
        ...
        if (targetIdx < defender.getSkillsLength() - 1 && defender.skill[targetIdx + 1] === "速")
            counterSpeed += 1;
        if (defender.kakugo === StatusFlag.ACTIVE)
            counterSpeed += 2;
        if (counterSpeed >= attackerSpeed || useIdx === -1) {
            ...
            if (defender.limited[targetIdx] === StatusFlag.ACTIVE)
            ...
            for (let i = 0; i < cDamage; i++) {
                ...
                if (defender.skill[targetIdx] === "交") {
                    if (useIdx === -1) {
                        this.log(`＞＞${attacker.playerName}の【弱撃】0はダメージの対象にならない！`);
                        break;
                    }
                    cTarget = useIdx;
                }
                    ...
                    for (let j = 0; j < attacker.getSkillsLength(); j++) {
                        if (attacker.type[j] !== Player_1.Player.NONE && attacker.scar[j] !== 2) {
                            cTarget = j;
                            break;
                        }
                    }
    ...
    applyExtremeHpDamage(pc, targetIdx, damage, logPrefix) {
    ...
    reserveSkillEffects(attacker, defender, idx) {
        const s = attacker.skill[idx];
        if (s === "毒")
            defender.tyudoku = StatusFlag.RESERVED;
        if (s === "焦")
            defender.yakedo = StatusFlag.RESERVED;
        if (s === "覚")
            attacker.kakugo = StatusFlag.RESERVED;
        if (s === "防")
            attacker.bouheki_ = StatusFlag.RESERVED;
        if (s === "封") {
            defender.stan = StatusFlag.RESERVED;
            defender.roubai = StatusFlag.RESERVED;
            defender.suijaku = StatusFlag.RESERVED;
        }
        if (s === "紫")
            attacker.stan = StatusFlag.RESERVED;
        if (s === "呪")
        ...
        if (s === "盗" || attacker.tozoku === 1) {
            ...
            if (defender.kakugo === StatusFlag.ACTIVE) {
                attacker.kakugo = StatusFlag.ACTIVE;
                defender.kakugo = 0;
            }
            if (defender.bouheki > 0) {
                attacker.bouheki += defender.bouheki;
                defender.bouheki = 0;
            }
            if (defender.musou === StatusFlag.ACTIVE) {
                attacker.musou = StatusFlag.ACTIVE;
                defender.musou = 0;
            }
            if (defender.sensei === StatusFlag.ACTIVE) {
                attacker.sensei = StatusFlag.ACTIVE;
                defender.sensei = 0;
            }
            if (defender.kaifuku > 0) {
                attacker.kaifuku = StatusFlag.ACTIVE;
                defender.kaifuku = 0;
            }
            if (defender.chiyu.length > 0) {
                if (attacker.chiyu.length === 0)
                    attacker.chiyu = [defender.chiyu[0]];
                defender.chiyu = [];
                defender.chiyuReserved = [];
            }
            if (defender.kyousou > 0) {
                attacker.kyousou += defender.kyousou;
                defender.kyousou = 0;
            }
            if (defender.reika > 0) {
                attacker.reika = StatusFlag.ACTIVE;
                defender.reika = 0;
            }
            attacker.tozoku = 0;
        }
    }
    reserveCounterEffects(attacker, defender, targetIdx) {
        ...
        if (s === "疫")
            attacker.ekibyo = StatusFlag.RESERVED;
        if (s === "紫")
            defender.stan = StatusFlag.RESERVED;
        if (s === "呪")
            attacker.suijaku = StatusFlag.RESERVED;
        if (s === "水")
            this.grantBouheki(defender, 1);
        if (s === "転")
            defender.roubai = StatusFlag.RESERVED;
        if (s === "受") {
            ...
            if (defender.tyudoku > 0) {
                attacker.tyudoku = StatusFlag.RESERVED;
                defender.tyudoku = 0;
            }
            if (defender.yakedo > 0) {
                attacker.yakedo = StatusFlag.RESERVED;
            ...
            if (defender.shimon > 0) {
                attacker.shimon += defender.shimon;
                defender.shimon = 0;
            }
            if (defender.shitto > 0) {
                attacker.shitto = StatusFlag.RESERVED;
                defender.shitto = 0;
            }
            if (defender.shikiyoku > 0) {
                attacker.shikiyoku = StatusFlag.RESERVED;
                defender.shikiyoku = 0;
            }
     ...
     * エフェクトの適用
     */
    applyEffects(pc1, pc2) {
        let flag = 0;
        const hasShitto = (pc1.shitto > 0);
        if (pc1.kakugo === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため覚悟を受け取れない！`);
            else {
                this.log(`${pc1.playerName}に覚悟が与えられた！`);
                pc1.kakugo = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.bouheki_ === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため防壁を受け取れない！`);
            else {
                this.log(`${pc1.playerName}に防壁が与えられた！`);
                pc1.bouheki += 3;
                flag = 1;
            }
            pc1.bouheki_ = 0;
        ...
        if (pc1.stan === StatusFlag.RESERVED) {
            this.log(`${pc1.playerName}はスタンを受けた！`);
            pc1.stan = StatusFlag.ACTIVE;
            flag = 1;
        }
        if (pc1.roubai === StatusFlag.RESERVED) {
            this.log(`${pc1.playerName}は狼狽を受けた！`);
            pc1.roubai = StatusFlag.ACTIVE;
            flag = 1;
        }
        ...
        if (pc1.tyudoku === StatusFlag.RESERVED) {
            this.log(`${pc1.playerName}は毒を受けた！`);
            pc1.tyudoku = StatusFlag.ACTIVE;
            flag = 1;
        }
        if (pc1.yakedo === StatusFlag.RESERVED) {
            this.log(`${pc1.playerName}は火傷を受けた！`);
            pc1.yakedo = StatusFlag.ACTIVE;
            flag = 1;
        }
        if (pc1.kaifuku === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため回復状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は回復状態になった！`);
                pc1.kaifuku = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        ...
        if (pc1.kyousou_ === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため協奏状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}に協奏が1つ与えられた！`);
                pc1.kyousou += 1;
                flag = 1;
            }
            pc1.kyousou_ = 0;
        ...
        if (pc1.reika === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため霊化状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は霊化状態になった！`);
                pc1.reika = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.gouyoku === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため強欲状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は強欲状態になった！`);
                pc1.gouyoku = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.gouman === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため傲慢状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は傲慢状態になった！`);
                pc1.gouman = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.funnu === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため憤怒状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は憤怒状態になった！`);
                pc1.funnu = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.boushoku === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため暴食状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は暴食状態になった！`);
                pc1.boushoku = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.taida === StatusFlag.RESERVED) {
            if (hasShitto)
                this.log(`${pc1.playerName}は嫉妬のため怠惰状態を受け取れない！`);
            else {
                this.log(`${pc1.playerName}は怠惰状態になった！`);
                pc1.taida = StatusFlag.ACTIVE;
                flag = 1;
            }
        }
        if (pc1.shitto === StatusFlag.RESERVED) {
            this.log(`${pc1.playerName}は嫉妬を受けた！`);
            pc1.shitto = StatusFlag.ACTIVE;
            flag = 1;
        }
        if (pc1.shikiyoku === StatusFlag.RESERVED) {
            this.log(`${pc1.playerName}は色欲を受けた！`);
            pc1.shikiyoku = StatusFlag.ACTIVE;
            flag = 1;
        }
        ...
        if (pc1.ekibyo === StatusFlag.RESERVED) {
            for (let i = 0; i < pc1.getSkillsLength(); i++) {
                if (pc1.type[i] !== Player_1.Player.NONE) {
                    if (pc1.skill[i] === "極") {
                    ...
                    pc1.skill[i] = "疫";
                    pc1.name[i] = "疫病";
                    pc1.type[i] = Player_1.Player.COUNTER;
                    pc1.speed[i] = pc1.getSkillLevel(i);
                    pc1.damage[i] = 0;
                    pc1.limited[i] = 0;
                    break;
                }
    ...
    applyBreakup(pc) {
        ...
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.scar[i] === 1) {
                ...
                // ＋硬による保護
                if (pc.type[i] !== Player_1.Player.ENCHANT && i < pc.getSkillsLength() - 1 && pc.skill[i + 1] === "硬") {
    ...
    destroySkill(pc, idx) {
        pc.skill[idx] = "＿";
        pc.name[idx] = "　　";
        pc.type[idx] = Player_1.Player.NONE;
        pc.speed[idx] = 0;
        pc.damage[idx] = 0;
        pc.scar[idx] = 0;
        pc.limited[idx] = 0;
     ...
     */
    executeRengeki() {
        const applyRengeki = (pc, opponent, pNo, isFirst) => {
            for (let i = 0; i < pc.getSkillsLength(); i++) {
                if (pc.skill[i] === "連") {
                    this.log();
                    this.log(`${pc.playerName}の【${pc.name[i]}】${i + 1}が発動！`);
    ...
    phaseEnd() {
        ...
        // 状態の解除
        if (this.pc1.musou === StatusFlag.ACTIVE) {
            this.log(`${this.pc1.playerName}の無想が解除された！`);
            this.pc1.musou = 0;
            endFlag = 1;
        }
        if (this.pc2.musou === StatusFlag.ACTIVE) {
            this.log(`${this.pc2.playerName}の無想が解除された！`);
            this.pc2.musou = 0;
            endFlag = 1;
        }
        if (this.pc1.sensei === StatusFlag.ACTIVE) {
            this.log(`${this.pc1.playerName}の先制が解除された！`);
            this.pc1.sensei = 0;
            endFlag = 1;
        }
        if (this.pc2.sensei === StatusFlag.ACTIVE) {
            this.log(`${this.pc2.playerName}の先制が解除された！`);
            this.pc2.sensei = 0;
            endFlag = 1;
        }
        // 毒のダメージ
        ...
        if (this.applyBreakup(this.pc1) + this.applyBreakup(this.pc2) >= 1)
            endFlag = 1;
        // 回復の処理
    ...
    applyKaifuku(pc) {
        ...
        for (let idx = 0; idx < pc.getSkillsLength(); idx++) {
            if (pc.name[idx] == "　　") {
                pc.skill[idx] = "Ｈ";
                pc.name[idx] = "ＨＰ";
                pc.type[idx] = Player_1.Player.ENCHANT;
            }
        }
    ...
    applyChiyu(pc) {
        ...
        if (pc.chiyu.length !== beforeCount)
            flag = 1;
        return flag;
    }
    restoreFirstDestroyedSkillAsHp(pc) {
        for (let idx = 0; idx < pc.getSkillsLength(); idx++) {
            if (pc.type[idx] === Player_1.Player.NONE && pc.skill[idx] === "＿") {
                pc.skill[idx] = "Ｈ";
                pc.name[idx] = "ＨＰ";
                pc.type[idx] = Player_1.Player.ENCHANT;
                pc.speed[idx] = 0;
    ...
    applyTaida(pc) {
        ...
        let count = 0;
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.type[i] === Player_1.Player.COUNTER)
                count++;
        }
        if (count > 0) {
        ...
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.skill[i] === "凍" && pc.limited[i] === StatusFlag.FROZEN) {
                ...
                for (let j = 0; j < opponent.getSkillsLength(); j++) {
                    if (opponent.type[j] !== Player_1.Player.NONE && opponent.scar[j] === 0) {
    ...
    applySuijaku(pc) {
        if (pc.suijaku !== StatusFlag.ACTIVE)
            return 0;
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.type[i] !== Player_1.Player.NONE && pc.skill[i] !== "空") {
                if (pc.skill[i] === "極") {
                ...
                pc.skill[i] = "空";
                pc.name[i] = "空白";
                pc.type[i] = Player_1.Player.ENCHANT;
                pc.speed[i] = 0;
                pc.damage[i] = 0;
                pc.scar[i] = 0;
                pc.limited[i] = 0;
    ...
    judgeMastery() {
        ...
        if (result === BattleResult.PC1_WIN)
            this.log(`${this.pc1.playerName}は全てを掌握した！！`);
        else if (result === BattleResult.PC2_WIN)
            this.log(`${this.pc2.playerName}は全てを掌握した！！`);
        else
            this.log("互いの【掌握】により、引き分け！");
        this.log();
        this.log("==========================================");
        return result;
    }
    hasActiveSkill(pc, skill) {
        for (let i = 0; i < pc.getSkillsLength(); i++) {
            if (pc.skill[i] === skill && pc.type[i] !== Player_1.Player.NONE)
                return true;
        }
        return false;
    }
     ...
     */
    judge(prior) {
        const isDead = (pc) => pc.type.every(t => t === Player_1.Player.NONE);
        ...
        if (result === BattleResult.PC1_WIN)
            this.log(`${this.pc1.playerName}の勝利！`);
        else if (result === BattleResult.PC2_WIN)
            this.log(`${this.pc2.playerName}の勝利！`);
